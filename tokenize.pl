...
tokenize_term(String,Term) :-
     ...
     ;
       ...
       tcl_eval(Interp,format('dialog .d {Syntax Error} {Trailing material deleted.~2n~s<here>~s} warning 0 {OK} {Cancel}',[Init,Rest]),"0")
     ).

portray_missing_material(Char,Bs,Cs) :-
       append(As,Bs,Cs),
       my_tk_interpreter(Interp),
       tcl_eval(Interp,format('dialog .d {Syntax Error} {Missing "~w" inserted.~2n~s<here>~s} warning 0 {OK} {Cancel}',[Char,As,Bs]),"0").
